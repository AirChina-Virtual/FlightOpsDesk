...
public sealed class TokenProvider(Func<CancellationToken, Task<AccessToken>> acquire)
{
    readonly SemaphoreSlim gate = new(1); AccessToken? cached;
    public async Task<string> GetAsync(CancellationToken ct)
    {
        await gate.WaitAsync(ct);
        try { if (cached is null || cached.ExpiresAt <= DateTimeOffset.UtcNow.AddMinutes(1)) cached = await acquire(ct); return cached.Value; }
        finally { gate.Release(); }
    }
    public void Invalidate() => cached = null;
    public static TokenProvider ClientCredentials(HttpClient client, OperationsCredentials credentials) => new(async ct =>
    {
        ...
        return new AccessToken(token, DateTimeOffset.UtcNow.AddSeconds(seconds));
    });
}
...
public sealed class OperationsTransport(HttpClient client, Uri origin, string vaKey, TokenProvider tokens)
{
    ...
    sealed class Budget { public SemaphoreSlim Gate { get; } = new(1); public DateTimeOffset Next; }
    static readonly ConcurrentDictionary<string, Budget> Budgets = new();
    ...
    public async Task<JsonDocument> SendAsync(HttpMethod method, Uri uri, object? body, CancellationToken ct)
    {
        ...
        var budget = Budgets.GetOrAdd(vaKey, _ => new());
        ...
        for (int attempt = 0; ; attempt++)
        {
            ...
            try
            {
                var delay = budget.Next - DateTimeOffset.UtcNow;
                if (delay > TimeSpan.Zero) await Task.Delay(delay, ct);
                var token = await tokens.GetAsync(ct);
                budget.Next = DateTimeOffset.UtcNow.AddSeconds(1);
                ...
                catch (HttpRequestException) when (read && attempt < 3) { budget.Next = DateTimeOffset.UtcNow.AddSeconds(Math.Pow(2, attempt)); continue; }
                catch (TaskCanceledException) when (read && !ct.IsCancellationRequested && attempt < 3) { budget.Next = DateTimeOffset.UtcNow.AddSeconds(Math.Pow(2, attempt)); continue; }
                if (response.Headers.TryGetValues("X-RateLimit-Remaining", out var remaining) && remaining.FirstOrDefault() == "0") budget.Next = DateTimeOffset.UtcNow.AddMinutes(1);
                if (response.StatusCode == HttpStatusCode.TooManyRequests)
                {
                    ...
                    budget.Next = DateTimeOffset.UtcNow + (retry > TimeSpan.Zero ? retry : TimeSpan.FromSeconds(1));
                }
            }
            ...
            using (response)
            {
                ...
                { await Task.Delay(TimeSpan.FromSeconds(Math.Pow(2, attempt)), ct); continue; }
